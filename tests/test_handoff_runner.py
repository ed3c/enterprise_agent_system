...
SPEC = importlib.util.spec_from_file_location("eas_h4r_runner", MODULE_PATH)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError("unable to load H4R runner")
runner = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(runner)


class H4RRunnerTests(unittest.TestCase):
    def queue(self):
    ...
    def test_contract_and_plan_bind_queue_v4_without_execution(self):
        contract = self.contract()
        queue = self.queue()
        runner.validate_runner_contract(contract)
        plan = runner.build_plan(queue, contract)
        self.assertEqual(plan["schema_version"], "enterprise-agent-system/local-handoff-runner-plan/v3")
        ...
        self.assertEqual(plan["superseded_queue_v3_authority"], "NONE")
        self.assertEqual(plan["superseded_runner_v3_authority"], "NONE")
        self.assertEqual(plan["eas_a_authority"], "ADVISORY_ONLY")
        self.assertTrue(plan["execute_requires_external_runner_admission"])
        self.assertEqual(len(plan["command_ids"]), 9)
        self.assertEqual(plan["cleanup_ids"], ["REMOVE_ROOT_D_WORKTREE", "PRUNE_WORKTREES", "DELETE_TEMP_ROOT_D_REF"])
    ...
    def test_queue_v3_is_refused(self):
        queue = self.queue()
        queue["schema_version"] = "enterprise-agent-system/local-handoff-queue/v3"
        with self.assertRaisesRegex(runner.RunnerError, "QUEUE_SCHEMA"):
            runner.validate_queue(queue)

    def test_old_queue_cannot_be_runner_parent(self):
        contract = self.contract()
        contract["queue_subject"]["commit"] = runner.SUPERSEDED_QUEUE_V3
        with self.assertRaisesRegex(runner.RunnerError, "QUEUE_PARENT_DRIFT"):
            runner.validate_runner_contract(contract)

    def test_queue_parent_receipt_drift_is_refused(self):
        contract = self.contract()
        contract["queue_subject"]["shadow_review"] = 1
        with self.assertRaisesRegex(runner.RunnerError, "QUEUE_PARENT_RECEIPT_DRIFT"):
            runner.validate_runner_contract(contract)
    ...
    def test_reused_core_cannot_gain_execution_authority(self):
        contract = self.contract()
        contract["core_reuse"]["execution_authority"] = "CURRENT"
        with self.assertRaisesRegex(runner.RunnerError, "CORE_REUSE_AUTHORITY"):
            runner.validate_runner_contract(contract)

    def test_eas_a_cannot_become_canonical_or_git_parent(self):
        contract = self.contract()
        contract["eas_a"]["authority"] = "CANONICAL"
        with self.assertRaisesRegex(runner.RunnerError, "EAS_A_AUTHORITY"):
            runner.validate_runner_contract(contract)
        contract = self.contract()
        contract["eas_a"]["relationship"] = "GIT_PARENT"
        with self.assertRaisesRegex(runner.RunnerError, "EAS_A_AUTHORITY"):
            runner.validate_runner_contract(contract)
    ...
    def test_google_or_source_correctness_promotion_is_refused(self):
        ...
        with self.assertRaisesRegex(runner.RunnerError, "GOOGLE_PROMOTION"):
            runner.validate_runner_contract(contract)
        contract = self.contract()
        contract["eas_a"]["source_correctness"] = "SUPPORTED"
        with self.assertRaisesRegex(runner.RunnerError, "SOURCE_CORRECTNESS_PROMOTION"):
            runner.validate_runner_contract(contract)
    ...
    def test_old_runner_subject_is_never_admitted(self):
        with tempfile.TemporaryDirectory() as root_dir:
            with self.assertRaisesRegex(runner.RunnerError, "SUPERSEDED_RUNNER_AUTHORITY_NONE"):
                runner.assert_admitted_runner_subject(pathlib.Path(root_dir), runner.SUPERSEDED_RUNNER_V3, "1" * 40)
    ...
    def test_path_escape_is_refused(self):
        with tempfile.TemporaryDirectory() as root_dir:
            ...
            with self.assertRaisesRegex(runner.RunnerError, "PATH_ESCAPE"):
                runner.resolve_env_path({"env": "EAS_WORKTREES", "relative": "../escape"}, env, {"EAS_WORKTREES"})
    ...
    def test_unknown_environment_is_refused(self):
        with tempfile.TemporaryDirectory() as root_dir:
            ...
            with self.assertRaisesRegex(runner.RunnerError, "ENV_NOT_ALLOWED"):
                runner.resolve_env_path({"env": "UNDECLARED", "relative": "."}, env, {"EAS_WORKTREES"})

    def test_relative_environment_root_is_refused(self):
        with self.assertRaisesRegex(runner.RunnerError, "ENV_ROOT_NOT_ABSOLUTE"):
            runner.resolve_env_path({"env": "EAS_WORKTREES", "relative": "."}, {"EAS_WORKTREES": "relative-root"}, {"EAS_WORKTREES"})

    def test_execution_roots_must_be_pairwise_disjoint(self):
        with tempfile.TemporaryDirectory() as root_dir:
            root = pathlib.Path(root_dir)
            checkout = root / "checkout"; checkout.mkdir()
            worktrees = checkout / "worktrees"; worktrees.mkdir()
            receipt = root / "receipt"; receipt.mkdir()
            env = {"EAS_CHECKOUT": str(checkout), "EAS_WORKTREES": str(worktrees), "EAS_RECEIPT_DIR": str(receipt)}
    ...
    def test_sanitized_environment_drops_unrelated_secret_names(self):
        source = {
            "PATH": os.environ.get("PATH", ""), "HOME": "/safe-home", "EAS_CHECKOUT": "/repo",
            "API_TOKEN": "must-not-survive", "PASSWORD": "must-not-survive",
        }
    ...
    def test_render_arg_accepts_declared_env_path_only(self):
        with tempfile.TemporaryDirectory() as root_dir:
            env = {"EAS_WORKTREES": root_dir}
            rendered = runner.render_arg({"env_path": {"env": "EAS_WORKTREES", "relative": runner.WORKTREE_RELATIVE}}, env, {"EAS_WORKTREES"})
    ...
    def test_command_result_uses_hermetic_subprocess_and_digest_only(self):
        with tempfile.TemporaryDirectory() as root_dir:
            ...
            spec = {
                "command_id": "HERMETIC_OK", "cwd": {"env": "FIXTURE_ROOT", "relative": "."},
                "argv": [{"literal": sys.executable}, {"literal": "-c"}, {"literal": "print('fixture-ok')"}],
                "timeout_seconds": 30,
            }
            code, out_digest, err_digest, failure = runner.command_result(
                spec, environ=env, allowlist={"FIXTURE_ROOT"}, child_env=runner.sanitized_child_env(env, ["FIXTURE_ROOT"])
            )
    ...
    def test_command_failure_stays_failure_candidate(self):
        with tempfile.TemporaryDirectory() as root_dir:
            env = {"FIXTURE_ROOT": root_dir}
            spec = {
                "command_id": "HERMETIC_FAIL", "cwd": {"env": "FIXTURE_ROOT", "relative": "."},
                "argv": [{"literal": sys.executable}, {"literal": "-c"}, {"literal": "raise SystemExit(7)"}],
                "timeout_seconds": 30,
            }
    ...
    def test_clean_residue_requires_no_dir_registration_ref_and_same_dirty_state(self):
        clean = {
            "root_d_worktree_exists": False, "root_d_worktree_registered": False,
            "temporary_root_d_ref_present": False, "checkout_dirty_state": "CLEAN",
        }
        ...
        for key in ("root_d_worktree_exists", "root_d_worktree_registered", "temporary_root_d_ref_present"):
            mutated = dict(clean); mutated[key] = True
            self.assertFalse(runner.clean_residue(mutated, "CLEAN"))
        dirty = dict(clean); dirty["checkout_dirty_state"] = "DIRTY:x"
    ...
    def synthetic_receipt(self):
        ...
        return {
            ...
            "started_at": "2026-08-20T00:00:00Z", "finished_at": "2026-08-20T00:00:01Z",
            "subject_before": subject, "subject_after": subject,
            "commands": [{"command_id": "fixture"}], "exit_codes": [0],
            "stdout_digests": [runner.digest_bytes(b"")], "stderr_digests": [runner.digest_bytes(b"")],
            "observed_commit": runner.ROOT_D_COMMIT, "observed_tree": runner.ROOT_D_TREE,
            "evidence_lane": "LOCAL_DETERMINISTIC", "result": "PASS",
            "dirty_state_before": "CLEAN", "dirty_state_after": "CLEAN",
            "residue_inventory": {}, "cleanup_result": "PASS", "failures": [], "retries": [],
    ...
    def test_receipt_validation_refuses_unknown_field(self):
        receipt = self.synthetic_receipt(); receipt["resolved_local_path"] = "/should/not/serialize"
    ...
    def test_pass_receipt_requires_exact_root_d_observation(self):
        receipt = self.synthetic_receipt(); receipt["observed_commit"] = "3" * 40
    ...
    def test_execute_requires_explicit_admitted_runtime(self):
        with self.assertRaisesRegex(runner.RunnerError, "RUNTIME_NOT_ADMITTED"):
            runner.execute_active(
                self.queue(), self.contract(), self.schema(), runtime_kind="UNADMITTED",
                admitted_runner_commit="3" * 40, admitted_runner_tree="4" * 40, environ={},
            )
    ...
    def test_preexisting_residue_blocks_before_any_command(self):
        with tempfile.TemporaryDirectory() as root_dir:
            ...
            residue = {
                "root_d_worktree_exists": False, "root_d_worktree_registered": False,
                "temporary_root_d_ref_present": True, "worktree_listing_digest": runner.digest_bytes(b"fixture"),
                "checkout_dirty_state": "CLEAN",
            }
            with (
                mock.patch.object(runner.core, "assert_exact_parent_bytes"),
                mock.patch.object(runner.core, "assert_admitted_runner_subject", return_value=subject),
                mock.patch.object(runner.core, "dirty_state", return_value="CLEAN"),
                mock.patch.object(runner.core, "residue_inventory", return_value=residue),
                mock.patch.object(runner.core, "command_result") as command,
            ):
                with self.assertRaisesRegex(runner.RunnerError, "PREEXISTING_RUNNER_RESIDUE_BLOCKED"):
                    runner.execute_active(
                        self.queue(), self.contract(), self.schema(), runtime_kind="CODEX_CLI_LOCAL",
                        admitted_runner_commit=subject["commit"], admitted_runner_tree=subject["tree"], environ=env,
                    )
    ...
    def test_main_failure_still_attempts_all_cleanup_and_writes_fail_receipt(self):
        with tempfile.TemporaryDirectory() as root_dir:
            ...
            clean = {
                "root_d_worktree_exists": False, "root_d_worktree_registered": False,
                "temporary_root_d_ref_present": False, "worktree_listing_digest": runner.digest_bytes(b"fixture"),
                "checkout_dirty_state": "CLEAN",
            }
            ...
            with (
                mock.patch.object(runner.core, "assert_exact_parent_bytes"),
                mock.patch.object(runner.core, "assert_admitted_runner_subject", return_value=subject),
                mock.patch.object(runner.core, "git_subject", return_value=subject),
                mock.patch.object(runner.core, "dirty_state", return_value="CLEAN"),
                mock.patch.object(runner.core, "residue_inventory", side_effect=[clean, clean]),
                mock.patch.object(runner.core, "command_result", side_effect=[main_failure, cleanup_ok, cleanup_ok, cleanup_ok]) as command,
                mock.patch.object(runner.core, "atomic_write_json") as write,
            ):
