...
The closeout SHA below is an immutable **public-stack integration baseline**, not a promise that the moving `main` branch will forever equal that SHA. At every fresh session, read the current repository HEAD/tree from Git before mutation.
...
`open canonical implementation PRs = 0` excludes a transient governance-documentation refresh or verification-only sibling. The public mechanisms are integrated and bounded delivery/verification metadata has been cleaned up. Program #1/#6, stronger-lane owners #5/#7/#15/#16/#17/#18, and Local Handoff #14 remain open because the source/PDF real-problem denominator is not closed.
...
```text
current repository HEAD/tree read from Git
...
The SHA of the Git object containing this file is the immutable identity of this version of AGENTS.md. Do not attempt to make a documentation file embed the SHA of its own future merge commit.
...
8. With `open canonical implementation PRs = 0`, do not manufacture a new mechanism PR unless a current open owner issue identifies a real uncovered mechanism or a verified freshness defect.
9. Do not hard-code a moving branch HEAD as permanent truth inside a file that will itself move that branch when merged; use immutable baseline subjects plus fresh Git readback.
